...
export default async function SmsTermsPage() {
  const content = await getMarkdownContent('sms-terms');

  return (
    <main className="min-h-screen bg-background">
      <div className="container mx-auto max-w-4xl px-4 py-12 sm:px-6 lg:px-8">
        <h1 className="text-4xl font-bold tracking-tight text-foreground font-headline">
          {content?.title ?? 'SMS Terms'}
        </h1>
        {content?.lastUpdated && (
        ...
        <div className="mt-10 border-t pt-6 text-sm text-muted-foreground flex items-center gap-4">
          ...
          <Link href="/privacy-policy" className="underline underline-offset-4">View Privacy Policy</Link>
        </div>
      </div>
    </main>
  );
